spec: task
name: "跳过场景不得判定为通过"
inherits: project
tags: [bootstrap, verify, lifecycle]
---

## Intent

把 `specwright` 的最终判定从“零失败即可通过”改成“关键场景被正向验证后才通过”，
避免把 `skip` 误报成绿色结果。

## Decisions

- 最终决策在出现 `skip` 或 `uncertain` 时返回非通过结果
- 验证报告继续保留每个场景原始的 `pass`、`fail`、`skip`、`uncertain` verdict
- 结构化约束检查继续产生 `pass` 或 `fail`

## Boundaries

### Allowed Changes
- src/spec_gateway/**
- src/**
- src/spec_core/verify.rs

### Forbidden
- 不要在存在 `skip` 的情况下返回 `passed: true`
- 不要把 `skip` 重写成 `pass`
- 不要隐藏导致非通过的 `skip` 或 `uncertain`

## Completion Criteria

Scenario: 单个跳过场景导致非通过
  Test: test_skip_is_not_passing
  Given 某个任务级 spec 只产生一个 `skip` 场景
  When lifecycle 输出 JSON 结果
  Then 最终字段 `passed` 为 `false`
  And 场景 verdict 仍然是 `skip`

Scenario: 结构通过加跳过仍然非通过
  Test: test_pass_plus_skip_is_not_passing
  Given 验证报告同时包含一个结构化 `pass` 和一个验收场景 `skip`
  When gateway 计算最终决策
  Then 结构化 `pass` 继续保留
  And 最终决策仍然为非通过
