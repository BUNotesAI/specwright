...
Scenario: 输出模式未覆盖时给出提示
  Test:
    Filter: test_output_mode_coverage_warns_when_json_or_output_flags_are_uncovered
  Given 某个 task spec 提到 `--json` 和 `-o/--output`
  When 该 spec 的场景只覆盖默认 human 输出
  Then lint 输出包含 `output-mode-coverage` warning
  And warning 指出未覆盖的输出模式

Scenario: 优先级和回退顺序未验证时给出提示
  Test:
...
Scenario: 外部 I/O 错误场景的测试强度过弱时给出提示
  Test:
