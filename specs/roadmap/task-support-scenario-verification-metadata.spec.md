...
Scenario: parser 解析中文验证强度元数据并保留到 AST
  Test:
    Filter: test_parse_scenario_verification_metadata_fields
  Given 某个场景在 `测试:` 后声明 `层级:`、`替身:` 与 `命中:`
  When parser 读取该 spec
  Then AST 中保留这些字段
  And 旧的 `测试:` selector 仍正常解析

Scenario: parser 同时支持英文验证强度元数据关键字
  Test:
    Filter: test_parse_english_verification_metadata_fields
  Given 某个英文 spec 在 `Test:` 后声明 `Level:`、`Test Double:` 与 `Targets:`
  When parser 读取该 spec
  Then AST 中保留这些字段
  And 英文关键字与中文关键字具有等价语义

Scenario: JSON 输出与 contract 渲染保留元数据
  Test:
...
Scenario: 旧规格文件保持兼容
  ...
  Given 现有仓库中的 task spec 都没有使用 `层级:`、`替身:` 或 `命中:`
  When 运行 parser 与 lint
  Then 这些旧 spec 继续通过
  And 不需要任何迁移即可使用新版本工具

Scenario: 高风险 I/O 场景缺少元数据时得到建议
  Test:
    Filter: test_lint_suggests_verification_metadata_for_external_io_scenarios
  Given 某个场景描述 HTTP、filesystem 或 protocol 错误路径
  When 该场景没有填写 `层级:`、`替身:` 或 `命中:`
  Then lint 给出建议性 warning
  And warning 解释这些字段有助于表达测试强度
